...
기준 시점: **2026-09-18**, v0.1.0 source `320f6ae`.
...
| Markdown 문서 무결성 | 통과 | 34개 파일의 상대 경로·대소문자·heading anchor를 dependency 설치 전 CI 검증 |
...
- [Alertmanager channel evidence PR CodeQL run 33552382306](https://github.com/sangmu1126/PipeLens/actions/runs/33552382306)
...
3. `ruff check .`
4. `mypy` strict mode로 `src/pipelens`, `ops`, `tests`의 모든 Python 코드 검사
5. 전체 `pytest -q`와 `pipelens`·`ops` statement/branch coverage 측정, XML·JSON artifact 14일 보관
...
2026-09-16에는 [무료 Quick Tunnel 재검증](acceptance-runs/2026-09-16-free-quick-tunnel.md)으로
OpenAI를 끄고 실제 OAuth callback, App installation, signed `workflow_run` HTTP 202, 5.47초
규칙 분석과 Commit Check 게시를 다시 통과했다. account-less endpoint의 HTTP permanent
redirect·HSTS·고정 hostname 부재는 그대로이므로 이 결과도 #62를 완료하지 않는다.
...
- [x] production secret manager·credential rotation의 redacted machine-readable evidence 도구
