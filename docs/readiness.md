...
로컬 `.venv`는 Python 3.14로 만들어져 있어 현재 dependency의 예정된 deprecation warning이
출력된다. 프로젝트 지원 기준과 CI 기준은 Python 3.12이며 해당 CI는 통과한다. Python 3.14
공식 지원을 선언하려면 별도의 compatibility 작업이 필요하다.
...
3. Python 3.13/3.14 compatibility matrix 여부 결정
