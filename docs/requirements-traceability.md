...
| --- | --- | --- | --- |
| 보안 | HMAC webhook, encrypted OAuth token, installation 접근 격리, LLM 전 마스킹, 외부 fork·미확인 context fail-closed 격리, production fail-closed 설정 | `tests/test_webhook.py`, `tests/test_auth.py`, `tests/test_sanitizer.py`, `tests/test_security.py`, `tests/test_pipeline.py`, `tests/test_github.py`, CodeQL·secret scan·dependency review | 실제 GitHub fork 경계 완료. context 실패·불완전 metadata는 자동 회귀 검증, production HTTPS [#62](https://github.com/sangmu1126/PipeLens/issues/62), secret manager [#65](https://github.com/sangmu1126/PipeLens/issues/65) |
